...
ggplot(count.hoods, aes(x=reorder(hood, count), y=count)) +
  ...
  xlab("Neighborhood") + 
  ggtitle("AirBnB Listings per Columbus, OH Neighborhood") +
  coord_flip()

# majority of listings are around campus or the city center
# should probably filter out the outlier listings

# we'll now perform a regression predicting price based on neighborhood
# we need dummy variables for the neighborhoods, using north campus as the base case

# quick list of neighborhoods to create dummy's for
count.hoods
x.hoods <- x %>% 
...
lm0 <- lm(Price~location_is_Near.South+
            ...
            location_is_Hilltop+
            location_is_South.Linden+
            location_is_Eastmoor_Walnut.Ridge+
            location_is_North.Linden,
          data=x.hoods)
summary(lm0)
anova(lm0)
plot(lm0)
...
# now we select significant variables using stepwise selection in various directions
stepAIC(lm0)
# lm(formula = Price ~ location_is_Downtown + location_is_Northland + 
#      location_is_Rocky.Fork_Blacklick + location_is_Northeast + 
#      location_is_Hilltop + location_is_Eastmoor_Walnut.Ridge, 
#    data = x.hoods)
stepAIC(lm0,direction="backward")
# same formula
lm.empty <- lm(Price~1,data=x.hoods)
stepAIC(lm.empty,direction="forward",scope=list(upper=lm0,lower=lm.empty))
# same formula
# forward, backward, and both, stepwise selection produce the same coefficients for selection

# try again with regsubsets
...
lm0.reg.sum$outmat # from data below, select the 6 model equation for min error
which.max(lm0.reg.sum$cp)
# R2 and MSE not useful, compare below to find optimal model selection
plot(lm0.reg.sum$adjr2,type="l")
plot(lm0.reg.sum$rss,type="l")
plot(lm0.reg.sum$cp,type="l") # looks like 5or 6 for low test error
plot(lm0.reg.sum$bic,type="l")
